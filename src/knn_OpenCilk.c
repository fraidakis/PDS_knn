...
void knnsearch_recursive(const Matrix *Q, int *idx, float *dst, int k, int MIN_SIZE, int sampling_reduction, int candidate_reduction, int CORPUS_CHUNK_SIZE, int HEIGHT, int depth, int mid_offset, float *squared_norms)
{
    ...
    else
    {
        ...
        cilk_spawn knnsearch_recursive(&Q1, idx, dst, k, MIN_SIZE, sampling_reduction, candidate_reduction, CORPUS_CHUNK_SIZE, HEIGHT, depth + 1, mid_offset, squared_norms);
        ...
        cilk_sync; // Wait for both recursive calls to finish
...
{
    cilk_for(int i = 0; i < sample_size; i++) // Parallel loop for sampled points
    {
...
{
    cilk_for(int i = 0; i < matrix->rows; i++)
    {
